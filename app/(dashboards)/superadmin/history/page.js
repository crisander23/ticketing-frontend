...
export default function SuperAdminHistoryPage() {
  ...
  const refreshAll = useCallback(() => mutate(), [mutate]);

  const processedTickets = useMemo(() => {
    let list = tickets || [];
    if (query.trim()) {
      const q = query.trim().toLowerCase();
      list = list.filter((t) => [t.title, t.status, String(t.ticket_id)].map((v) => (v ?? '').toString().toLowerCase()).some((s) => s.includes(q)));
    }
  ...
  return (
    <div className={`min-h-screen w-full ${bgClass}`}>
      ...
      <div className={`flex flex-col min-h-screen transition-all duration-300 ease-in-out ${isDesktopCollapsed ? 'md:pl-20' : 'md:pl-64'}`}>
        <header className={`sticky top-0 z-30 flex h-16 items-center justify-between px-4 md:hidden ${isDark ? 'bg-slate-900/70 border-b border-white/10 backdrop-blur' : 'bg-white/90 border-b border-slate-200 backdrop-blur'}`}>
          <h1 className={`text-lg font-semibold ${cardTitle}`}>Global History</h1>
        ...
        <main className="w-full px-4 sm:px-6 pt-4 md:pt-8 pb-8 space-y-6">
          <div className="hidden md:block">
            <h1 className={`text-3xl font-semibold ${cardTitle}`}>Global Ticket History</h1>
            <p className={`text-sm ${subTitle}`}>Audit logs of all ticket updates in the system.</p>
          </div>

          <div className={`${inputWrap} px-3 py-2`}>
            <input type="search" placeholder="Search history..." value={query} onChange={(e) => setQuery(e.target.value)} className={`w-full rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 ${inputField}`} />
          </div>

          <section>
             {isLoading ? <div className={`p-8 text-center opacity-70 ${subTitle}`}>Loading...</div> : error ? <div className="text-red-400">Failed to load.</div> : (
                <div className={tableWrap}>
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[600px] table-auto text-left">
                      <thead className={headClass}>
                        <tr><th className={thClass}>Title</th><th className={thClass}>Latest Action</th><th className={thClass}>Date</th></tr>
                      </thead>
                      <tbody>
                        {processedTickets.map((t) => (
                          <tr key={t.ticket_id} className={rowClass} onClick={() => router.push(`/tickets/${t.ticket_id}`)}>
                            <td className={tdClass}>
                              <div className="flex flex-col"><span className="font-medium text-base">{t.title}</span><span className={`text-xs ${subTitle} uppercase tracking-wider`}>#{t.ticket_id}</span></div>
                            </td>
                            <td className={tdClass}><StatusBadge status={t.status} /></td>
                            <td className={`${tdClass} whitespace-nowrap opacity-80`}>{new Date(t.updated_at || t.created_at).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
function StatusBadge({ status }) {
  ...
  let label = status;
  if (s === 'open') { colorClass = 'bg-blue-600 text-white'; label = 'Open'; } 
  else if (s === 'in_progress') { colorClass = 'bg-amber-500 text-slate-900'; label = 'In Progress'; } 
  else if (s === 'resolved') { colorClass = 'bg-violet-600 text-white'; label = 'Resolved'; } 
  else if (s === 'closed') { colorClass = 'bg-emerald-600 text-white'; label = 'Closed'; }
  return <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${colorClass}`}>{label}</span>;
}